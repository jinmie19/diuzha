...
class Task extends Base{
    ...
    public function index(){
        ...
        if($category_id){
            ...
            if($paixu == 2){
                $tasks =Db::name('task')->where($param)->order('update_time desc')->select();
            } elseif($paixu == 3){
                $tasks =Db::name('task')->where($param)->order('unit_price desc')->select();
            }else{
                $tasks = Db::name('task')->where($param)->select();
            }

        }else{ 
            if($paixu == 2){
                $tasks =Db::name('task')->order('update_time desc')->select();
            } elseif($paixu == 3){
                $tasks =Db::name('task')->order('unit_price desc')->select();
            }else{
              	if($keyword != ''){
                	$tasks = Db::name('task')->where($param)->select();
                }else{
                $tasks = Db::name('task')->select();}
            }
            $catename['title'] = '全部';
        }
    ...
    public function detail(){
       ...
       $whereparam=array();
      $whereparam['id']=$item['id'];
      $whereparam['is_display']=1;
      	$successNum= Db::name('task')->where($whereparam)->count();
